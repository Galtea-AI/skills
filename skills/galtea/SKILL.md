---
name: galtea
description: Interact with the Galtea platform -- the AI product testing and evaluation platform for AI/LLM products -- and access its documentation. Use when needing to (1) query or modify Galtea data via the `galtea` CLI, (2) run or monitor evaluations of an AI product (pre-deployment or production monitoring), (3) wire an AI product into Galtea via the Python SDK, or (4) look up Galtea docs, concepts, or CLI/SDK usage.
