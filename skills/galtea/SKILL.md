...
- **`--ids` + any other filter triggers a false 404.** Across every `list` endpoint that accepts `--ids` (evaluations, sessions, versions, products, metrics, …), the server checks `entitiesReturned.length` against the requested id count and throws `404 "<Entity>s not found: <id-list>"` when they differ. The check runs *after* every other filter has been applied, so combining `--ids id1,id2,id3` with `--statuses PENDING` (or `--from-created-at`, `--metric-ids`, etc.) yields a 404 listing every id whose row was excluded by the *other* filter -- even though the ids themselves exist. This is especially confusing in poll loops: once your evaluations leave `PENDING`, `list --ids "$IDS" --statuses PENDING` flips from `[]` to a 404 error object, and `jq 'length'` on that object returns `1` (one key: `message`), trapping a naive `while … -gt 0` in an infinite loop. Two safe patterns: **(a)** if you have a scope (versionId, sessionId, inferenceResultId, productId), filter by `--<scope>-ids` instead -- those are lenient and combine cleanly with `--statuses`; **(b)** if you only have raw ids, use `--ids` alone and filter status client-side via `jq '[.[] | select(.status == "...")] | length'`.
